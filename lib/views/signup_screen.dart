...
//         context,
//         '${_carImages.length} image(s) selected',
//       );
//     }
//   }
...
//       carType: _carTypeController.text.trim().isEmpty
//           ? 'Manual'
//           : _carTypeController.text.trim(),
//       fuel: _fuelController.text.trim().isEmpty
//           ? 'Petrol'
...
class _SignupScreenState extends State<SignupScreen> {
  ...
  Future<void> _pickCarImages() async {
    final result = await FilePicker.pickFiles(
      type: FileType.image,
      allowMultiple: true, // This allows multiple selection
    );
    ...
    if (result != null && result.files.isNotEmpty) {
      ...
      if (validFiles.isNotEmpty) {
        setState(() {
          // Add new images to existing ones (append, not replace)
          _carImages.addAll(validFiles);
        });
        ToastHelper.showSuccess(
          context,
          '${validFiles.length} image(s) added. Total: ${_carImages.length}',
        );
      }
    }
  }
